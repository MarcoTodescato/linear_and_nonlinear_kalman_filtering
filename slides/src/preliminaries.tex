...
\begin{block}{Random signal}    
    ...
    \begin{itemize}
        ...
        $S$ specifies the a-priori admissible trajectories for the signal.
    \end{itemize}
\end{block}
...
\begin{block}{Stochastic process}
    ...
    \begin{itemize}
        ...
        \item $\S$ set of all logical meaningful \underline{propositions or
        events} regarding the signal
...
%--------------------------
\myframe{Stochastic process: weather example}{%
...
Then we have that:
\begin{itemize}
    \item $S:=\{(\sigma,\sigma),(\sigma,\rho),(\rho,\sigma),(\rho,\rho)\}$ the trajectories.
    \pause
    \item $\S$ could be $\{E_1,E_2\}$ where
    \begin{itemize}
        \item $E_1$: \textit{it rains at least one day} $:= \{(\sigma,\rho),(\rho,\sigma),(\rho,\rho)\}$ 
        \item $E_2$: \textit{the first sunny day} $:= \{(\sigma,\sigma),(\sigma,\rho)\}$
    \end{itemize}
    \pause
    \item $P$: assume weather is independent across days with $P[\sigma]=0.7$ and $P[\rho]=0.3$. Then
    \begin{itemize}
        \item $P[E_1] = (0.7 * 0.3) + (0.3 * 0.7) + (0.3 * 0.3) = 0.51$ 
        \item $P[E_2] = 0.7^2 + (0.7 * 0.3) = 0.7$
    \end{itemize}    
\end{itemize}
}

%--------------------------
\myframe{Stochastic process: robot navigation example}{%

Assume there is a small robot moving on a 1D line. At each time it can either
\begin{itemize}
    \item move to the right: $+1$
    \item stay still: $0$
\end{itemize}
...
Then we have that:
\begin{itemize}
    \item $S:=\{(0,0,0),(0,0,1),(0,1,1),(0,1,2)\}$ the trajectories.
    \pause
    \item $\S$ could be $\{E_1,E_2\}$ where
    \begin{itemize}
        \item $E_1$: \textit{the robot moves at $t=1$} $:= \{(0,1,1),(0,1,2)\}$ 
        \item $E_2$: \textit{the robot never moves} $:= \{(0,0,0)\}$
    \end{itemize}
    \pause
    \item $P$: assume independent movements $P[0]=0.7$ and $P[+1]=0.3$. Then
    \begin{itemize}
        \item $P[E_1] = (0.3 * 0.7) + (0.3 * 0.3) +  = 0.3$ 
        \item $P[E_2] = 0.7^2 = 0.49$
    \end{itemize}  
\end{itemize}

\vspace{0.5cm}

\textbf{Obviously this can easily be made more complicated to describe more meaningful events.}
}
...
Our definition $(S,\S,P)$ is a \tq{concrete path-based} version of the abstract
$(\Omega,\A,P)$. If we define a variable $y(t)\in(S,\S,P)$ as
%
$$
y(t,s) := s(t)
$$
...
develop useful tools for filtering and identification.

\medskip
For this reason, it is important to understand that, regardless the real nature
of the underline signal, the same formulation (as \textit{stochastic process})
can indeed be used to describe every signal (aleatoric or not).

\medskip
On the other hand, it might be worth remembering that this choice is dictated by
the \tq{simplicity}, compactness and generality to describe events that could
indeed also be described using a \tq{deterministic} formulation.
...
\myframe{Conditional Expectation: brief recap}{%
    ...
    \begin{itemize}
        ...
        functions. Also it is unbiased, i.e., $\E[g(\y)]=\E[\x]$ (from law of
        total expectation. Check for the Gaussian case);
...
\myframe{Geometric formulation}{%
    \begin{exampleblock}{Theorem: Orthogonal Projections (scalar form)}
        ...
        $$
            \x-\z^* \bot\ \H(\y)\ \ \equiv\ \ \E[(\x-\z^*)\y_i]=0\ \ i\in\until{m}
        $$
