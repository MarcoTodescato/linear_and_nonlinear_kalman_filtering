...
\myframe{Asymptotic Behavior: error dynamics}{%
  ...
  %
  \pause
  After update we get an error dynamics governed by
  %
  \begin{align*}    
  ...
  \pause
  \vspace{.5cm}
  \textbf{Essentially, the gain $K_t$, covariance $P_t$ and closed-loop error
  $\Gamma_t$ dynamics are governed by the resolution of the discrete-time
  Riccati Equation (R.E.)}
...
Wiener-Kolmogorov theories as well as the asymptotic stability of the filer.}
...
\begin{exampleblock}{}
  ...
  \end{align*}  
  And if $(\Gamma_\infty, K_\infty)$ reachable, then $\Gamma_\infty$ is
