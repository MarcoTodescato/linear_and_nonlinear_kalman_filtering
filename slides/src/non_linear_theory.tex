...
\begin{itemize}
    \item The linearized model is used only to compute matrix-related update, $P(\cdot|\cdot), \Lambda, L$
    ...
    computation of $F, H$ and $\Phi$ at every step, hence $L$ cannot be precomputed
    offline.
...
$$
used to propagate the prior estimated state in the filter equations
%
$$
...
\myframe{Unscented Kalman Filter (UKF)}{%
    \begin{itemize}
        \item Uses sigma points to capture higher-order
        moments\footnote{\url{https://en.wikipedia.org/wiki/Unscented_transform}}: for
        any random vector $\x$, sigma points are a set of any $N$ vectors
        satisfying:
        \begin{itemize}
...
code rather than on slides.} \texttt{9\_nonlinear\_ukf.ipynb}
